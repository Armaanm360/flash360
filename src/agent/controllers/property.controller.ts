...
class ClientPropertyController extends AbstractController {
  ...
  // get all eminities
  public getAllTopics = this.asyncWrapper.wrap(
    // { arrayBodySchema: this.ActivityValidator.createActivity },
    {},
    async (req: Request, res: Response) => {
  ...
  public getAllTopicsWiseQuest = this.asyncWrapper.wrap(
    // { arrayBodySchema: this.ActivityValidator.createActivity },
    {},
    async (req: Request, res: Response) => {
      const { code, ...data } = await this.service.getAllTopicsWiseQuest(req);
      res.status(code).json(data);
    }
  );
  // get all eminities
  public createTopic = this.asyncWrapper.wrap(
    // { arrayBodySchema: this.ActivityValidator.createActivity },
    {},
    async (req: Request, res: Response) => {
      const { code, ...data } = await this.service.createTopic(req);
      res.status(code).json(data);
    }
  );
  public createQuiz = this.asyncWrapper.wrap(
    // { arrayBodySchema: this.ActivityValidator.createActivity },
    {},
    async (req: Request, res: Response) => {
      const { code, ...data } = await this.service.createQuiz(req);
      res.status(code).json(data);
    }
  );
