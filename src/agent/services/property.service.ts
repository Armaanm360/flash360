...
class AgentPropertyService extends AbstractServices {
  ...
  public async createProperty(req: Request) {
    return this.db.transaction(async (trx) => {
      ...
      // Get the highest number from the fieldname
  ...
  public async createTopic(req: Request) {
    return this.db.transaction(async (trx) => {
      ...
      const data = await model.createTopic({
        name: req.body.name,
  ...
  public async createQuiz(req: Request) {
    return this.db.transaction(async (trx) => {
      const model = this.Model.propertyModel(trx);
      ...
      //     success: false,
      //     message: 'Invalid input. Topic ID and questions are required.',
      //   });
      // }
      ...
      await model.createQuestion(formattedQuestions);
  ...
  public async getAllTopicsWiseQuest(req: Request) {
    const model = this.Model.agentModel(); // Get the property model
    const id = Number(req.params.id);

    const data = await model.getAllTopicsWiseQuestion(id);
