...
class ClientPropertyService extends AbstractServices {
  ...
  public async getAllTopics(req: Request) {
    const model = this.Model.agentModel(); // Get the property model

    const data = await model.getAllTopics();
  ...
  public async insertAnswer(req: Request) {
    ...
    if (!Array.isArray(answers) || answers.length === 0) {
      return {
        success: false,
        code: 400,
        message: 'Invalid request data. Expected an array of answers.',
      };
    }
    ...
    for (const answer of answers) {
      if (!answer.question_id || !answer.answer) {
        return {
          success: false,
          code: 400,
          message: 'Each entry must have question_id and answer.',
        };
      }
    ...
    return {
      success: true,
      code: 200,
      message: 'Your inquiries have been submitted successfully.',
    };
  }
  //listing property
