...
import { IAdminUser, IAgent, TDB } from '../../common/types/commonTypes';
import {
...
class AgentModel extends Schema {
  ...
  public async getSingleTeacher({
    ...
    const query = this.db('admin_user')
  ...
  public async getSingleUser({ email, id }: { email?: string; id?: number }) {
    // Start building the query
    const query = this.db('participants')
      .withSchema(this.USER_SCHEMA)
      .select('*');
  ...
  //register  agent
  public async createTeacher(payload: IAdminUser) {
    const data = await this.db('admin_user')
      .withSchema(this.USER_SCHEMA)
      .insert(payload, 'id');
  ...
  public async createParticipant(payload: any) {
    const data = await this.db('participants')
      .withSchema(this.USER_SCHEMA)
      .insert(payload, 'id');
  ...
  public async insertAnswer(payload: any) {
    const data = await this.db('answers')
      .withSchema(this.USER_SCHEMA)
      .insert(payload);
  ...
  public async getAllTopics() {
    const data = this.db('topics').withSchema(this.USER_SCHEMA).select('*');

    const total = await this.db('topics')
      .withSchema(this.USER_SCHEMA)
      .count('id AS total');
  ...
  public async getAllTopicsWiseQuestion(id: number) {
  ...
  public async getSingleProfileAgent(id: Number) {
    return await this.db('admin_user')
      .withSchema(this.USER_SCHEMA)
      .where({ id: id });
  }
