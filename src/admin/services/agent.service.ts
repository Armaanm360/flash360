...
class AdminAgentService extends AbstractServices {
  constructor() {
    super();
  }
  ...
  // get all agents
  public async getAllAgents(req: Request) {
    const model = this.Model.agentModel();
    const { data, total } = await model.getAllAgents({ ...req.query });
  ...
  // get all agents
  public async getSingleAgent(req: Request) {
    const model = this.Model.agentModel();
  ...
  public async updateSingleAgentVerification(req: Request) {
    return this.db.transaction(async (trx) => {
      const id = req.params.agent_id;
      const model = this.Model.agentModel(trx);
      const checkAgent = await model.getSingleProfileAgent(Number(id));
      ...
      await model.addVerificationTrack({
        admin_id: req.admin.id,
        agent_id: Number(id),
        verification_field: verification_field, // First key from req.body
        verification_status: verification_status, // First value from req.body
      });

      await model.updateSingleAgentVerification(Number(id), req.body);
  ...
  public async getAllAmenity(req: Request) {
    const model = this.Model.propertyModel();
    const { data, total } = await model.getAllAmenity(req.agent.id, {
      ...req.query,
    });
